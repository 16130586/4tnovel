...
@WebServlet("/manage/account/dashboard-vols")
public class AccountDashBoardVols extends HttpServlet {
	private static final long serialVersionUID = 1L;

	public AccountDashBoardVols() {
		super();
	}

	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		Account hostAc = (Account) request.getSession().getAttribute("account");
		Connection cnn = (Connection) request.getAttribute("connection");
		NovelDAO novelDao = new NovelDAO(cnn);
		try {
			List<Novel> novels = novelDao.getNovels(null, " IDOWNER=" + hostAc.getId(), 0, Integer.MAX_VALUE);
		...
		request.getServletContext().getRequestDispatcher("/jsps/pages/account-new.all-vol.jsp").forward(request, response);
	}
